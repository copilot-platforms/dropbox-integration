...
const MappingTableRow = () => {
  ...
  const { folderTree, tempMapList } = useUserChannel()
      ...
      {tempMapList.map((mapItem, index) => (
        <tr key={`${generateRandomString(8)}-${index}`}>
          <td className="whitespace-nowrap px-6 py-2">
            <CopilotSelector
              name="File channel"
              initialValue={mapItem.fileChannelDetail}
              onChange={(val) => onUserSelectorValueChange(val, index)}
            />
          </td>
          <td className="whitespace-nowrap px-6 py-2">
            <TreeSelect
              value={filteredValue?.[index] || mapItem.dbxRoothPath}
          ...
          <td className="whitespace-nowrap px-6 py-2">
            ...
            ) : !mapItem.dbxRoothPath || !mapItem.fileChannelDetail.length ? (
